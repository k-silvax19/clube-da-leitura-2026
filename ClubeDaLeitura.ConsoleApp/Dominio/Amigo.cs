...
public class Amigo : EntidadeBase
{
    ...
    public override string[] Validar()
    {
        ...
        if (string.IsNullOrWhiteSpace(Nome))
            ...
            erros += erros = "O Campo \"Nome\" deve ser entre 2 e 100 caracteres;";

        if (string.IsNullOrWhiteSpace(NomeResponsavel))
            erros += "O Campo \"Nome Do Responsável\" deve ser entre 2 e 100 caracteres;";
        ...
        if (contadorDigitos < 10 || contadorDigitos > 11)
            erros += erros += "O Campo \"Telefone\" deve ser entre 10 e 100 digitos;";

        if (contemLetraOuSimbolo)
        {
            erros += erros += "O Campo \"Telefone\" deve ser digitos;";
        }
